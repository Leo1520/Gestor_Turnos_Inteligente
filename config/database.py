...
# Soportar modo MySQL local (XAMPP) si se desea
USE_MYSQL = os.getenv('USE_MYSQL', 'false').lower() in ('1', 'true')

if USE_MYSQL:
    # En modo MySQL usamos SQLAlchemy desde config/mysql_database.py
    def get_supabase_client():
        raise RuntimeError("Proyecto configurado para usar MySQL local. No hay cliente Supabase disponible.")
